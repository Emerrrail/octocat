    ...
    return (
        <div>
            {!errorRepo && <div className='repoPage'>
                ...
                <Button text='Visit Github' url={repoDetail.svn_url} />
            </div>}
            {errorRepo && <div>{errorRepo.status === 404 && <Error message={'OOPS! REPO NOT FOUND'} />}{errorRepo.status === 403 && <Error message={'OOPS! API RATE HAS BEEN EXCEEDED. PLEASE WAIT A WHILE AND RELOAD THE PAGE.'} />}{!errorRepo.status && <Error message={'NETWORK ERROR'} />}</div>}
        </div>
    )
}
